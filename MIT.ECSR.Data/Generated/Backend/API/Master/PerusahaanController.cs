...
using MIT.ECSR.Core.Perusahaan.Command;
...
namespace MIT.ECSR.API.Controllers
{
    public partial class PerusahaanController : BaseController<PerusahaanController>
    {
        ...
        public async Task<IActionResult> Add([FromBody] PerusahaanRequest request)
        {
            var add_request = _mapper.Map<AddPerusahaanRequest>(request);
            add_request.Inputer = Inputer;
            return Wrapper(await _mediator.Send(add_request));
        }

        [HttpPut(template: "edit/{id}")]
        public async Task<IActionResult> Edit(Guid id, [FromBody] PerusahaanRequest request)
        {
            var edit_request = _mapper.Map<EditPerusahaanRequest>(request);
            edit_request.Id = id;
        ...
        {
            return Wrapper(await _mediator.Send(new DeletePerusahaanRequest() { Id = id, Inputer = Inputer }));
        }
