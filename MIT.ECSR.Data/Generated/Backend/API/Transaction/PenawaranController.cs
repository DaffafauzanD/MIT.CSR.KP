...
namespace MIT.ECSR.API.Controllers
{
    public partial class PenawaranController : BaseController<PenawaranController>
    {
        ...
        {
            var list_request = _mapper.Map<GetPenawaranListRequest>(request);
            return Wrapper(await _mediator.Send(list_request));
        }

        [HttpPost(template: "add")]
        public async Task<IActionResult> Add([FromBody] PenawaranRequest request)
        {
            var add_request = _mapper.Map<AddPenawaranRequest>(request);
            add_request.Inputer = Inputer;
            return Wrapper(await _mediator.Send(add_request));
        }

        [HttpPut(template: "edit/{id}")]
        public async Task<IActionResult> Edit(Guid id, [FromBody] PenawaranRequest request)
        {
            var edit_request = _mapper.Map<EditPenawaranRequest>(request);
            edit_request.Id = id;
        ...
        {
            return Wrapper(await _mediator.Send(new DeletePenawaranRequest() { Id = id, Inputer = Inputer }));
        }
