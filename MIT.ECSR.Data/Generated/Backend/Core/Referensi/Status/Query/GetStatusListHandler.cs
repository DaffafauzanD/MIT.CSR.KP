...
{
    public class GetStatusListRequest : ListRequest,IListRequest<GetStatusListRequest>,IRequest<ListResponse<StatusResponse>>
    {
    }
    internal class GetStatusListHandler : IRequestHandler<GetStatusListRequest, ListResponse<StatusResponse>>
    {
        private readonly ILogger _logger;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork<ApplicationDBContext> _context;
        public GetStatusListHandler(
            ILogger<GetStatusListHandler> logger,
            IMapper mapper,
        ...
        public async Task<ListResponse<StatusResponse>> Handle(GetStatusListRequest request, CancellationToken cancellationToken)
        {
            ListResponse<StatusResponse> result = new ListResponse<StatusResponse>();
            try
            {
				var query = _context.Entity<MIT.ECSR.Data.Model.RefStatus>().AsQueryable();

				#region Filter
				Expression<Func<MIT.ECSR.Data.Model.RefStatus, object>> column_sort = null;
				List<Expression<Func<MIT.ECSR.Data.Model.RefStatus, bool>>> where = new List<Expression<Func<MIT.ECSR.Data.Model.RefStatus, bool>>>();
				...
				result.List = _mapper.Map<List<StatusResponse>>(data_list);
            ...
            {
                _logger.LogError(ex, "Failed Get List Status", request);
                result.Error("Failed Get List Status", ex.Message);
            }
            return result;
        }

        #region List Utility
		private (Expression<Func<MIT.ECSR.Data.Model.RefStatus, bool>> where, Expression<Func<MIT.ECSR.Data.Model.RefStatus, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<MIT.ECSR.Data.Model.RefStatus, object>> result_order = null;
			Expression<Func<MIT.ECSR.Data.Model.RefStatus, bool>> result_where = null;
            if (!string.IsNullOrWhiteSpace(search) && !string.IsNullOrWhiteSpace(field))
            {
                ...
                switch (field)
                {
					...
					case "value" : 
						if(is_where){
							if (int.TryParse(search, out var _Value))
								result_where = (d=>d.Value == _Value);
						}
						else
