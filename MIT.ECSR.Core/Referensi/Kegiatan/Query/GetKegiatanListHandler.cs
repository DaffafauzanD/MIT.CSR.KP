...
{
    public class GetKegiatanListRequest : ListRequest,IListRequest<GetKegiatanListRequest>,IRequest<ListResponse<KegiatanResponse>>
    {
    }
    internal class GetKegiatanListHandler : IRequestHandler<GetKegiatanListRequest, ListResponse<KegiatanResponse>>
    {
        private readonly ILogger _logger;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork<ApplicationDBContext> _context;
        public GetKegiatanListHandler(
            ILogger<GetKegiatanListHandler> logger,
            IMapper mapper,
        ...
        public async Task<ListResponse<KegiatanResponse>> Handle(GetKegiatanListRequest request, CancellationToken cancellationToken)
        {
            ListResponse<KegiatanResponse> result = new ListResponse<KegiatanResponse>();
            try
            {
				var query = _context.Entity<MIT.ECSR.Data.Model.RefKegiatan>().AsQueryable();

				#region Filter
				Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, object>> column_sort = null;
				List<Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, bool>>> where = new List<Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, bool>>>();
				...
				result.List = _mapper.Map<List<KegiatanResponse>>(data_list);
            ...
            {
                _logger.LogError(ex, "Failed Get List Kegiatan", request);
                result.Error("Failed Get List Kegiatan", ex.Message);
            }
            return result;
        }

        #region List Utility
		private (Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, bool>> where, Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, object>> result_order = null;
			Expression<Func<MIT.ECSR.Data.Model.RefKegiatan, bool>> result_where = null;
