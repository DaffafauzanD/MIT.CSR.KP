...
namespace MIT.ECSR.Core.Dati.Query
{
    ...
    internal class GetDatiListHandler : IRequestHandler<GetDatiListRequest, ListResponse<DatiResponse>>
    {
        ...
        public async Task<ListResponse<DatiResponse>> Handle(GetDatiListRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
				var query = _context.Entity<MIT.ECSR.Data.Model.RefDati>().AsQueryable();

				#region Filter
				Expression<Func<MIT.ECSR.Data.Model.RefDati, object>> column_sort = null;
				List<Expression<Func<MIT.ECSR.Data.Model.RefDati, bool>>> where = new List<Expression<Func<MIT.ECSR.Data.Model.RefDati, bool>>>();
        ...
        #region List Utility
		private (Expression<Func<MIT.ECSR.Data.Model.RefDati, bool>> where, Expression<Func<MIT.ECSR.Data.Model.RefDati, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<MIT.ECSR.Data.Model.RefDati, object>> result_order = null;
			Expression<Func<MIT.ECSR.Data.Model.RefDati, bool>> result_where = null;
            if (!string.IsNullOrWhiteSpace(search) && !string.IsNullOrWhiteSpace(field))
            {
                ...
                switch (field)
                {
					...
					case "kodedati1" : 
						if(is_where){
							result_where = (d=>d.KodeDati1.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.KodeDati1);
					break;
					case "kodedati2" : 
						if(is_where){
							result_where = (d=>d.KodeDati2.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.KodeDati2);
					break;
					case "kodedati3" : 
						if(is_where){
							result_where = (d=>d.KodeDati3.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.KodeDati3);
					break;
					case "kodedati4" : 
						if(is_where){
							result_where = (d=>d.KodeDati4.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.KodeDati4);
					break;
					case "namadati1" : 
						if(is_where){
							result_where = (d=>d.NamaDati1.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.NamaDati1);
					break;
					case "namadati2" : 
						if(is_where){
							result_where = (d=>d.NamaDati2.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.NamaDati2);
					break;
					case "namadati3" : 
						if(is_where){
							result_where = (d=>d.NamaDati3.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.NamaDati3);
					break;
					case "namadati4" : 
						if(is_where){
							result_where = (d=>d.NamaDati4.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.NamaDati4);
					break;
					case "updateby" : 
						if(is_where){
