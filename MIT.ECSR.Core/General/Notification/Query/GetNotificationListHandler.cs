...
namespace MIT.ECSR.Core.Notification.Query
{
    ...
    internal class GetNotificationListHandler : IRequestHandler<GetNotificationListRequest, ListResponse<NotificationResponse>>
    {
        ...
        public async Task<ListResponse<NotificationResponse>> Handle(GetNotificationListRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
				var query = _context.Entity<MIT.ECSR.Data.Model.TrsNotification>().Where(x => x.IdUser == request.IdUser).AsQueryable();

				#region Filter
				Expression<Func<MIT.ECSR.Data.Model.TrsNotification, object>> column_sort = null;
				List<Expression<Func<MIT.ECSR.Data.Model.TrsNotification, bool>>> where = new List<Expression<Func<MIT.ECSR.Data.Model.TrsNotification, bool>>>();
				...
				result.Count = await query_count.Where(x => !x.IsOpen).CountAsync();
				result.OK();
            }
            catch (Exception ex)
            {
        ...
        #region List Utility
		private (Expression<Func<MIT.ECSR.Data.Model.TrsNotification, bool>> where, Expression<Func<MIT.ECSR.Data.Model.TrsNotification, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<MIT.ECSR.Data.Model.TrsNotification, object>> result_order = null;
			Expression<Func<MIT.ECSR.Data.Model.TrsNotification, bool>> result_where = null;
